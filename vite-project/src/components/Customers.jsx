...
const baseURL =  "http://localhost:8081/v1/customer" ;
...
const handleAddCustomer= async(e)=>{
 ...
 const customer = {name : name, email : email};  
  ...
  return (
    ...
    <div className="row">
  <div className="col">
    <input type="text" name="name" value={name} className="form-control" onChange={(e)=>setName(e.target.value)}  placeholder="First name" />
  </div>
  <div className="col">
    <input type="text" name="email"  value={email} className="form-control" onChange={(e)=>setEmail(e.target.value)} placeholder="Email" />
  </div>
  <div className="col">
    <button type="submit" onClick={handleAddCustomer}  className="btn btn-primary">Submit</button>
  </div>
    </div>
    </form>
  ...
  {customerList.map((c,key)=>(
    <tr key={key}>
      ...
      <td>
       ...
       <td><button disabled = {getUserRoles() == 'USER'} onClick={()=>handleDeleteCustomer(c.customerId)} className="btn btn-warning"
        title={getUserRoles() === 'USER' ? "You don't have permission to delete customers" : ""}>Delete</button></td>
      <td>
        <button onClick={()=>goToEdit(c.customerId)} className="btn btn-success">
          Edit
        </button>
      </td>
